...
void TTrack::SetUp(std::string root_dir, const ClassifierType classifier_type, const CameraType camera_type){
  ...
  try{
    ...
    switch(camera_type_){
    case STEREO:
      tracker_.reset(new StereoToolTracker(3.4,50,*root_dir_ + "/config/camera.xml"));
      break;
    case MONOCULAR:
      tracker_.reset(new MonocularToolTracker(3.4,50,*root_dir_ + "/config/mono_camera.xml"));
      break;
    default:
      tracker_.reset(new StereoToolTracker(3.4,50,*root_dir_ + "/config/camera.xml"));
      break;
