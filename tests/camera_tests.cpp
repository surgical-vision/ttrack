...
namespace ttrk {
  ...
  namespace test {
    ...
    class TestMonocularCamera : public ttrk::MonocularCamera {

    public:
      
      explicit TestMonocularCamera(const std::string &calib_filename) : MonocularCamera(calib_filename) {}
...
BOOST_AUTO_TEST_CASE(camera_load_test) {
  ...
  ttrk::test::TestMonocularCamera camera("../../data/lnd/camera/mono_camera.xml");
  const cv::Mat unprojected_points = camera.GetUnprojectedImagePlane(640,480);
  ...
  for (int r = 1; r < unprojected_points.rows-1; ++r){
    for (int c = 1; c < unprojected_points.cols-1; ++c){
      ...
      const cv::Point2d projected_point = camera.ProjectPoint(cv::Vec3d(point[0], point[1], 1));
